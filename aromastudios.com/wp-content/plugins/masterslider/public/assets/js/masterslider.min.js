...
window.averta = {},
    ...
    function($) {
        ...
        p.setStartAnim = function(anim) {
            ...
            if ($.extend(this.end_anim, this._parseEff(this.end_anim.name)), this.slider = this.slide.slider, this.masked && (this.$mask = $("<div></div>").addClass("ms-layer-mask"), this.link ? (this.link.wrap(this.$mask), this.$mask = this.link.parent()) : (this.$element.wrap(this.$mask), this.$mask = this.$element.parent()), this.maskWidth && this.$mask.width(this.maskWidth), this.maskHeight && (this.$mask.height(this.maskHeight), -1 === this.__cssConfig.indexOf("height") && this.__cssConfig.push("height"))), this.layerOrigin = this.$element.data("origin"), this.slide.slider.options.responsive) {
                var el = this.$element;
                this.origin = [el.data("phone-origin"), el.data("tablet-origin"), el.data("origin")], this.offsetX = [el.data("phone-offset-x"), el.data("tablet-offset-x"), el.data("offset-x")], this.offsetY = [el.data("phone-offset-y"), el.data("tablet-offset-y"), el.data("offset-y")], this.relativeScale = el.data("relative-size"), this.hideOn = (el.data("hide-on") || "").split(",")
            } else {
            ...
            if (this.lastBp !== bp && (this.lastBp = bp, this.layerOrigin = this.slide.slider.getResponsiveValue(this.origin), this.layerOrigin)) {
                var vOrigin = this.layerOrigin.charAt(0),
                    ...
                    layerEle = this.masked ? this.$mask[0] : this.$element[0];
                switch (this.middleAlign = !1, this.centerAlign = !1, this.baseStyle.top = void 0, this.baseStyle.left = void 0, this.baseStyle.right = void 0, this.baseStyle.bottom = void 0, layerEle.style.top = "", layerEle.style.left = "", layerEle.style.right = "", layerEle.style.bottom = "", void 0 === offsetY && (offsetY = 0), vOrigin) {
                    case "t":
                        layerEle.style.top = offsetY + "px", this.baseStyle.top = offsetY;
                        break;
                    case "b":
                        layerEle.style.bottom = offsetY + "px", this.baseStyle.bottom = offsetY;
                        break;
                    case "m":
                        layerEle.style.top = offsetY + "px", this.baseStyle.top = offsetY, this.middleAlign = !0
                }
                switch (void 0 === offsetX && (offsetX = 0), hOrigin) {
                    case "l":
                        layerEle.style.left = offsetX + "px", this.baseStyle.left = offsetX;
                        break;
                    case "r":
                        layerEle.style.right = offsetX + "px", this.baseStyle.right = offsetX;
                        break;
                    case "c":
                        layerEle.style.left = offsetX + "px", this.baseStyle.left = offsetX, this.centerAlign = !0
                }
            }
        }, p.locate = function() {
    ...
    function($) {
        ...
        p.create = function() {
            ...
            window.MSBrowserInfo.msie && this.$element.on("dragstart", function(event) {
        ...
        p.__playVideo = function() {
            ...
            if (has_img || this.$element.data("btn")) {
                ...
                if (this.video_btn = $("<div></div>").appendTo(this.$element).addClass("ms-video-btn").on("click", function() {
                    ...
                    window.MSBrowserInfo.msie && this.img.on("dragstart", function(event) {
                        event.preventDefault()
                    })
                }
            }
    ...
    function() {
        ...
        p.create = function() {
            ...
            for (var key, tempValue, i = 0; 4 > i; i++) key = positionKies[i], key in this.baseStyle && void 0 !== this.baseStyle[key] && (tempValue = this.$element.css(key), this.$element.css(key, ""), this.$container.css(key, tempValue));
    ...
    function(window, document, $) {
        ...
        p.onSwipeStart = function() {
            ...
            var that = this;
            this.hasLayers && this.layerController.create(), this.link && this.link.addClass("ms-slide-link").html("").on("click", function(e) {
    ...
    function(document, window) {
        ...
        if (window.MasterSlider) {
            ...
            p.init = function() {
                var api = this.slider.api,
                    view = api.view;
                this.filters = this.slider.options.filters, this.slideList = view.slideList, this.slidesCount = view.slidesCount, this.dimension = view[view.__dimension], this.target = "slide" === this.slider.options.filterTarget ? "$element" : "$bg_img", this.filterName = window.MSBrowserInfo.webkit ? "WebkitFilter" : "filter";
                ...
                for (var factor, slide, i = 0; i < this.slidesCount; ++i) slide = this.slideList[i], factor = Math.min(1, Math.abs(value - slide.position) / this.dimension), slide[this.target] && (window.MSBrowserInfo.msie ? null != this.filters.opacity && slide[this.target].opacity(1 - this.filters.opacity * factor) : slide[this.target][0].style[this.filterName] = this.generateStyle(factor))
